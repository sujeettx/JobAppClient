...
const UpdateCompanyProfile = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://jobappserver-1.onrender.com/users/${userId}`);
  ...
  const handleSubmit = async () => {
    try {
      await axios.patch(`https://jobappserver-1.onrender.com/users/${userId}`, { profile: profileData });
